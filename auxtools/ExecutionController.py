...
class ExecutionController():
    ...
    def write_to_log(self,text,mode='a'):
        if self.use_controller == 1:
            f = open('{}/{}'.format(self.TMP, self.filename), mode)
    ...
    def start_new_execution(self):
        ...
        if self.use_controller==1:
            ...
            self.write_to_log('Starting Execution {}'.format(self.last_execution_id),'w')
            self.send_log_to_s3()
            new_line = [
    ...
    def finish_execution(self):
        if self.use_controller==1:
            ...
            query_2 = '''UPDATE {} set finish ='{}' where {} = {} and (status = 'SUCCESS' or status = 'FAILED')'''.format(self.control_table,self.finish,self.id_field,self.last_execution_id)
            cursor = self.cnx.cursor()
